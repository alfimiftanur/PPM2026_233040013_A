...
// ─────────────────────────────────────────────
// Exception khusus untuk error jaringan
// ─────────────────────────────────────────────
...
class ApiClient {
  ...
  // === Base URL & API key (dari kontrak API) ===
  ...
  /// POST /api/catatan — Buat catatan baru
  ...
  /// PUT /api/catatan/{id} — Update catatan
  ...
  /// Mengirim request HTTP dan menangani 3 jenis error:
  /// 1. SocketException — tidak ada internet
  /// 2. TimeoutException — server tidak merespons
  /// 3. HTTP 4xx/5xx — validasi/server error
  Future<http.Response> _send(Future<http.Response> Function() req) async {
    try {
      final res = await req().timeout(_timeout);
      // Sukses jika status 2xx
      if (res.statusCode >= 200 && res.statusCode < 300) return res;
      // Error jika bukan 2xx — throw ApiException
      throw ApiException(res.statusCode, _extractMessage(res));
    } on SocketException {
      // Error: tidak ada internet atau koneksi terputus
      throw ApiException(0, 'Tidak ada koneksi internet.');
    } on TimeoutException {
      // Error: server tidak merespons dalam 10 detik
      throw ApiException(0, 'Server tidak merespons (timeout).');
    }
    // ApiException dari status code akan dipropagasi otomatis
  }

  /// Extract pesan error dari response body JSON
